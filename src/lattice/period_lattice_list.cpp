...
int PeriodLatticeList::get1nn3(_type_lattice_coord x, _type_lattice_coord y, _type_lattice_coord z,
  ...
  if (x % 2 == 0) {
    latti_id = getId(x - 1, y - 1, z - 1);
    if(latti_id == source_id) _1nn_list[0].type = atom_type;
    else getnnlattice(latti_id, &_1nn_list[0]);

    latti_id = getId(x - 1, y - 1, z);
    if(latti_id == source_id) _1nn_list[1].type = atom_type;
    else getnnlattice(latti_id, &_1nn_list[1]);

    latti_id = getId(x - 1, y, z - 1);
    if(latti_id == source_id) _1nn_list[2].type = atom_type;
    else getnnlattice(latti_id, &_1nn_list[2]);

    latti_id = getId(x - 1, y, z);
    if(latti_id == source_id) _1nn_list[3].type = atom_type;
    else getnnlattice(latti_id, &_1nn_list[3]);

    latti_id = getId(x + 1, y - 1, z - 1);
    if(latti_id == source_id) _1nn_list[4].type = atom_type;
    else getnnlattice(latti_id, &_1nn_list[4]);

    latti_id = getId(x + 1, y - 1, z);
    if(latti_id == source_id) _1nn_list[5].type = atom_type;
    else getnnlattice(latti_id, &_1nn_list[5]);

    latti_id = getId(x + 1, y, z - 1);
    if(latti_id == source_id) _1nn_list[6].type = atom_type;
    else getnnlattice(latti_id, &_1nn_list[6]);

    latti_id = getId(x + 1, y, z);
    if(latti_id == source_id) _1nn_list[7].type = atom_type;
    else getnnlattice(latti_id, &_1nn_list[7]);
...
int PeriodLatticeList::get2nn3(_type_lattice_coord x, _type_lattice_coord y, _type_lattice_coord z,
  ...
  latti_id = getId(x - 2, y, z);
  if(latti_id == source_id) _2nn_list[0].type = atom_type;
  else getnnlattice(latti_id, &_2nn_list[0]);
  
  // _2nn_list[0] = &_lattices[z + 0][y + 0][x - 2];
  latti_id = getId(x, y - 1, z);
  if(latti_id == source_id) _2nn_list[1].type = atom_type;
  else getnnlattice(latti_id, &_2nn_list[1]);
  
  // _2nn_list[1] = &_lattices[z + 0][y - 1][x + 0];
  latti_id = getId(x, y, z - 1);
  if(latti_id == source_id) _2nn_list[2].type = atom_type;
  else getnnlattice(latti_id, &_2nn_list[2]);
  
  // _2nn_list[2] = &_lattices[z - 1][y + 0][x + 0];
  latti_id = getId(x, y, z + 1);
  if(latti_id == source_id) _2nn_list[3].type = atom_type;
  else getnnlattice(latti_id, &_2nn_list[3]);
  
  // _2nn_list[3] = &_lattices[z + 1][y + 0][x + 0];
  latti_id = getId(x, y + 1, z);
  if(latti_id == source_id) _2nn_list[4].type = atom_type;
  else getnnlattice(latti_id, &_2nn_list[4]);
  
  // _2nn_list[4] = &_lattices[z + 0][y + 1][x + 0];
  latti_id = getId(x + 2, y, z);
  if(latti_id == source_id) _2nn_list[5].type = atom_type;
  else getnnlattice(latti_id, &_2nn_list[5]);
  
  // _2nn_list[5] = &_lattices[z + 0][y + 0][x + 2];
  return 6;
}

void PeriodLatticeList::getRandomLattice(const _type_lattice_coord& x, const _type_lattice_coord& y, const _type_lattice_coord& z,
...
void PeriodLatticeList::getnnlattice(_type_lattice_id latti_id, Lattice *_1nn_list_lattice) {
  ...
  if      (vac_hash.count(latti_id))  _1nn_list_lattice->type = LatticeTypes{LatticeTypes::V};
  else if (re_hash.count(latti_id))   _1nn_list_lattice->type = LatticeTypes{LatticeTypes::Re};
  else if (mn_hash.count(latti_id))   _1nn_list_lattice->type = LatticeTypes{LatticeTypes::Mn};
  else if (ni_hash.count(latti_id))   _1nn_list_lattice->type = LatticeTypes{LatticeTypes::Ni};
  else if (si_hash.count(latti_id))   _1nn_list_lattice->type = LatticeTypes{LatticeTypes::Si};
  else if (momo_hash.count(latti_id)) _1nn_list_lattice->type = LatticeTypes{LatticeTypes::MoMo};
  else if (more_hash.count(latti_id)) _1nn_list_lattice->type = LatticeTypes{LatticeTypes::MoRe};
  else if (rere_hash.count(latti_id)) _1nn_list_lattice->type = LatticeTypes{LatticeTypes::ReRe};
  else                                _1nn_list_lattice->type = LatticeTypes{LatticeTypes::Mo};
}
