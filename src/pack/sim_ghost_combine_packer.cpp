...
void SimGhostCombinePacker::onSendCombine(ChangeLattice *buffer, 
  ...
  for (const auto& pair : now_exchange_surface_z) {
    ...
    if (lats->vac_hash.count(pair)) {
      buffer[len].type._type = LatticeTypes::V;
    } else if (lats->re_hash.count(pair)) {
      buffer[len].type._type = LatticeTypes::Re;
    } else if (lats->mn_hash.count(pair)) {
      buffer[len].type._type = LatticeTypes::Mn;
    } else if (lats->ni_hash.count(pair)) {
      buffer[len].type._type = LatticeTypes::Ni;
    } else if (lats->si_hash.count(pair)) {
      buffer[len].type._type = LatticeTypes::Si;
    } else if (lats->momo_hash.count(pair)) {
      buffer[len].type._type = LatticeTypes::MoMo;
    } else if (lats->more_hash.count(pair)) {
      buffer[len].type._type = LatticeTypes::MoRe;
    } else if (lats->rere_hash.count(pair)) {
      buffer[len].type._type = LatticeTypes::ReRe;
    } else {
      buffer[len].type._type = LatticeTypes::Mo;
    }
    len++;
  }
}
