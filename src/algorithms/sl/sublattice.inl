...
void SubLattice::startTimeLoop(Ins pk_inst, ModelAdapter<E> *p_model, EventHooks *p_event_hooks) {
  ...
  double time_commu_total = 0.0;
  double time_vac_calculate_total = 0.0;
  double time_selected_event_total = 0.0;
...
template <typename E>
void SubLattice::recbPerformWrapper(ModelAdapter<E> *p_model, const type_sector_id sector_id) {
  p_model->recb(p_domain->local_sector_region[sector_id], sector_id);
}
...
// template <class PKs, class Ins> void SubLattice::syncSimGohstRegionsCombine(Ins &pk_inst, std::array<std::unordered_set<_type_lattice_id>, 7>& exchange_ghost, 
...
//   int num_send[13];
//   int num_receive[13];
//   int num_send_surface[13];
//   int num_receive_surface[13];
//   ChangeLattice *send_buff;
//   ChangeLattice *receive_buff;
//   MPI_Status status;
//   MPI_Status status_surface;
//   MPI_Status send_statuses[13];
//   MPI_Status recv_statuses[13];
//   MPI_Status send_statuses_surface[13];
//   MPI_Status recv_statuses_surface[13];
//   MPI_Request send_requests[13];
//   MPI_Request send_requests_surface[13];
//   // 0, 7, 2, 5, 3, 4, 1, 6
//   // for (int d = 0; d < 13; d++) {
...
template <class PKs, class Ins> void SubLattice::syncSimRegions(Ins &pk_inst, std::array<std::unordered_set<_type_lattice_id>, 7>& exchange_ghost, 
                                                                std::array<std::unordered_set<_type_lattice_id>, 8>& exchange_surface_x,
                                                                std::array<std::unordered_set<_type_lattice_id>, 8>& exchange_surface_y, 
                                                                std::array<std::unordered_set<_type_lattice_id>, 8>& exchange_surface_z) {
  ...
  for (int d = 0; d < comm::DIMENSION_SIZE; d++) {
    int dim_id = d;
    std::unordered_set<_type_lattice_id>* p_current_surface = nullptr;
    if (dim_id == 0) p_current_surface = &exchange_surface_x[next_sector.id];
    else if (dim_id == 1) p_current_surface = &exchange_surface_y[next_sector.id];
    else if (dim_id == 2) p_current_surface = &exchange_surface_z[next_sector.id];
    ...
    num_send[dim_id] = packer.sendLength2(dim_id, exchange_ghost, *p_current_surface, &send_count);
    ...
    packer.onSend2(send_buff, exchange_ghost, dim_id, *p_current_surface, &send_count);
